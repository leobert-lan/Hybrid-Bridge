...
//        
        self.userInteractionEnabled=YES;//YES:hud拦截所有touch事件
...
#pragma mark - 😄触摸 loading
- (void)touchesEnded:(NSSet *)touches withEvent:(UIEvent *)event{
    //测试代码，看看范围
//    self.backgroundColor=RGBAHex(kColorAuxiliary101, .5);

//    UIWindow *win=[UIApplication sharedApplication].delegate.window;
    UITouch *et = [[event allTouches] anyObject];
    CGPoint ep;// = [et locationInView:win];
    CGRect navRect;//=(CGRect){0,0,100,64};//(CGRect){0,0,win.bounds.size.width,64}
...
////        
    ...
    CGFloat sz=120;
    ep = [et locationInView:self];
    navRect=(CGRect){self.indicator.center.x-sz/2,self.indicator.center.y-sz/2,sz,sz};
    if (CGRectContainsPoint(navRect, ep)) {
        DLog(@"HUD中心点击：%@",NSStringFromCGRect(navRect));
        if ([self.delegate respondsToSelector:@selector(hudCenterTouch:)]) {
            [self.delegate hudCenterTouch:self];
        }
    }
...
- (BOOL)pointInside:(CGPoint)point withEvent:(nullable UIEvent *)event {
    BOOL isInside = YES;//touche不传递到下一层
    UIWindow *win=[UIApplication sharedApplication].delegate.window;
    CGRect navRect=(CGRect){0,0,100,64};//(CGRect){0,0,win.bounds.size.width,64}
    if (([self superview].bounds.size.height>=win.bounds.size.height) && CGRectContainsPoint(navRect, point)) {
        DLog(@"导航栏left触发");
        if ([self.delegate respondsToSelector:@selector(hudStopByTouchNavLeft:)]) {
            [self.delegate hudStopByTouchNavLeft:self];
        }

        isInside = NO;//touche可穿透
    }
