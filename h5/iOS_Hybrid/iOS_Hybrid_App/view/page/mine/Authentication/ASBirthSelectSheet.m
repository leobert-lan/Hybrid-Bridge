...
- (void)makeUI {
    ...
    _containerView = [[UIView alloc] initWithFrame:CGRectMake(10, MainScreenHeight - 290 - 80, MainScreenWidth - 20, 305)];
    }
...
- (void)doneAction:(UIButton *)btn {
    if (self.GetSelectDate) {
        DLog(@"%@",_datePicker.date);
//        _GetSelectDate([self.formatter stringFromDate:_datePicker.date]);
        NSString *timeSp = [NSString stringWithFormat:@"%f",[_datePicker.date timeIntervalSince1970]];
        NSLog(@"timeSp:%@",timeSp);
        
        _GetSelectDate(_datePicker.date);
        [self removeFromSuperview];
    }
}
