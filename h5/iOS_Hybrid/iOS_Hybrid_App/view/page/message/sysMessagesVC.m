...
@interface sysMessagesVC ()<UITableViewDelegate,UITableViewDataSource,cloudBaseCellDelegate>
{
    BOOL isStyleSel;
    BOOL isAllSel;
...
- (void)UIGlobal
{
    ...
    vline = [[UIView alloc] initWithFrame:CGRectMake(APP_W / 2, APP_H - 94, 0.5, 50)];
    ...
    btndele.hidden = YES;
    btndele.frame = CGRectMake((APP_W / 2)+0.5, APP_H - 94, (APP_W / 2)-0.5, 50);
...
- (void)refreshList{
    ...
    [MessageAPI SystemMessageListType:@"system" offset:page success:^(NSMutableArray *data) {
        relistType = @"0";
        self.arrData=nil;
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (indexPath.row < self.arrData.count) {
        tableID = @"SysMessagesCell";
        SysMessagesCell *cell;
        cell = (SysMessagesCell*)[tableView dequeueReusableCellWithIdentifier:tableID];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (isStyleSel) {
        ...
        SysMessagesCell *cell = [tableView cellForRowAtIndexPath:indexPath];
        cell.vShowL.constant = 65;
        cell.lblDetailR.constant = -55;
        if (mm != nil) {
...
- (IBAction)btnDeleAction:(id)sender {
    ...
    UIAlertAction *can = [UIAlertAction actionWithTitle:@"确定" style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
        ...
        [MessageAPI MessageDeleteId:idStr success:^(id model) {
            ...
            [self showText:@"删除成功!"];
            if (self.sysMessagesBlock) {
                self.sysMessagesBlock();
            }
            isAllSel=NO;
