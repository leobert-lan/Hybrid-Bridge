...
import com.lht.cloudjob.mvp.model.bean.CheckUndertakePerssionResBean;
...
public class DemandInfoActivityPresenter extends ABSVerifyNeedPresenter implements
    ...
    public void callContribute() {
        ...
        if (bean.getModel() == MODEL_REWARD) {
            iDemandInfoActivity.jump2UndertakeReward(bean.getTask_bn());
        } else {
            if (bean.getIs_mark() == MARK_HIDE) {
                iDemandInfoActivity.jump2UndertakeHideBid(bean.getTask_bn());
            } else {
                iDemandInfoActivity.jump2UndertakeOpenBid(bean.getTask_bn());
            }
        }
    }
    ...
    class UndertakePermissionCallback implements ApiModelCallback<BaseVsoApiResBean> {
        @Override
        ...
        public void onFailure(BaseBeanContainer<BaseVsoApiResBean> beanContainer) {
            ...
            DLog.i(getClass(), "投稿权限校验失败，错误码：" + ret);

            if (CheckUndertakePerssionResBean.RETS_NEEDAUTH_P.contains(ret)) { //需要个人认证
                iDemandInfoActivity.showPAuthenticaDialog();
            } else if (CheckUndertakePerssionResBean.RETS_NEEDAUTH_E.contains(ret)) { //需要企业认证
                iDemandInfoActivity.showEAuthenticaDialog();
            } else if (CheckUndertakePerssionResBean.RETS_NEEDAUTH_PORE.contains(ret)) { //需要个人或者企业认证
                iDemandInfoActivity.showPorEAuthenticDialog();
            } else if (CheckUndertakePerssionResBean.RETS_NEEDAUTH_MOBILE.contains(ret)) {  //需要绑定手机号
                iDemandInfoActivity.showBindPhoneDialog();
            } else if (ret == CheckUndertakePerssionResBean.UNDERTAKE_COUNT_FULLED) { //投稿达到上限
    ...
    public void callSafePageRefresh() {
        iDemandInfoActivity.showWaitView(true);
